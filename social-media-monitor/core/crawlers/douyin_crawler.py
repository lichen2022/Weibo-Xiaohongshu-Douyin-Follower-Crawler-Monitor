...
class DouyinCrawler:
    ...
    def get_follower_count(self, sec_user_id: str) -> Optional[int]:
        ...
        try:
            ...
            self.logger.info(f"响应头: {dict(response.headers)}")
            self.logger.info(f"响应内容长度: {len(response.text)} 字节")
            self.logger.info(f"响应内容类型: {response.headers.get('Content-Type', 'unknown')}")
            self.logger.info(f"响应编码: {response.headers.get('Content-Encoding', 'none')}")
            ...
            try:
                ...
                self.logger.error(f"响应内容前1000字符: {response.text[:1000]}")
                self.logger.error(f"响应内容编码: {response.encoding}")
                self.logger.error(f"响应内容类型: {type(response.content)}")
                self.logger.error(f"响应原始内容长度: {len(response.content)} 字节")
                raise
